///////////////////////////////////////////////////////////////////////////////
//
// IAR ANSI C/C++ Compiler V8.22.2.15995/W32 for ARM      21/Apr/2018  09:33:35
// Copyright 1999-2018 IAR Systems AB.
//
//    Cpu mode     =  thumb
//    Endian       =  little
//    Source file  =  
//        C:\Users\Tim\Programming\Code\ARM_32\Patterns\02_HardwareAdapterPattern\Code\C\IO2Sensor.c
//    Command line =  
//        -f C:\Users\Tim\AppData\Local\Temp\EWF48C.tmp
//        (C:\Users\Tim\Programming\Code\ARM_32\Patterns\02_HardwareAdapterPattern\Code\C\IO2Sensor.c
//        -lC
//        C:\Users\Tim\Programming\Code\ARM_32\Patterns\02_HardwareAdapterPattern\Code\C\Debug\List
//        -lA
//        C:\Users\Tim\Programming\Code\ARM_32\Patterns\02_HardwareAdapterPattern\Code\C\Debug\List
//        -o
//        C:\Users\Tim\Programming\Code\ARM_32\Patterns\02_HardwareAdapterPattern\Code\C\Debug\Obj
//        --no_cse --no_unroll --no_inline --no_code_motion --no_tbaa
//        --no_clustering --no_scheduling --debug --endian=little
//        --cpu=Cortex-M4 --fpu=None --dlib_config "C:\Program Files (x86)\IAR
//        Systems\Embedded Workbench 8.0\arm\INC\c\DLib_Config_Normal.h" -On)
//    Locale       =  C
//    List file    =  
//        C:\Users\Tim\Programming\Code\ARM_32\Patterns\02_HardwareAdapterPattern\Code\C\Debug\List\IO2Sensor.s
//
///////////////////////////////////////////////////////////////////////////////

        RTMODEL "__SystemLibrary", "DLib"
        RTMODEL "__dlib_full_locale_support", "0"
        RTMODEL "__dlib_version", "6"
        AAPCS BASE,INTERWORK
        PRESERVE8
        REQUIRE8

        #define SHT_PROGBITS 0x1

        EXTERN free
        EXTERN malloc

        PUBLIC IO2Sensor_Create
        PUBLIC IO2Sensor_Destroy
        PUBLIC IO2Sensor_Init
        PUBLIC IO2Sensor_gimmeO2Conc
        PUBLIC IO2Sensor_gimmeO2Flow
        
          CFI Names cfiNames0
          CFI StackFrame CFA R13 DATA
          CFI Resource R0:32, R1:32, R2:32, R3:32, R4:32, R5:32, R6:32, R7:32
          CFI Resource R8:32, R9:32, R10:32, R11:32, R12:32, R13:32, R14:32
          CFI EndNames cfiNames0
        
          CFI Common cfiCommon0 Using cfiNames0
          CFI CodeAlign 2
          CFI DataAlign 4
          CFI ReturnAddress R14 CODE
          CFI CFA R13+0
          CFI R0 Undefined
          CFI R1 Undefined
          CFI R2 Undefined
          CFI R3 Undefined
          CFI R4 SameValue
          CFI R5 SameValue
          CFI R6 SameValue
          CFI R7 SameValue
          CFI R8 SameValue
          CFI R9 SameValue
          CFI R10 SameValue
          CFI R11 SameValue
          CFI R12 Undefined
          CFI R14 SameValue
          CFI EndCommon cfiCommon0
        
// C:\Users\Tim\Programming\Code\ARM_32\Patterns\02_HardwareAdapterPattern\Code\C\IO2Sensor.c
//    1 #include <stdlib.h>
//    2 #include "IO2Sensor.h"
//    3 

        SECTION `.text`:CODE:NOROOT(1)
          CFI Block cfiBlock0 Using cfiCommon0
          CFI Function IO2Sensor_Create
        THUMB
//    4 IO2Sensor* IO2Sensor_Create(void) {
IO2Sensor_Create:
        PUSH     {R4,LR}
          CFI R14 Frame(CFA, -4)
          CFI R4 Frame(CFA, -8)
          CFI CFA R13+8
//    5 	IO2Sensor* me = (IO2Sensor*)malloc(sizeof(IO2Sensor));
        MOVS     R0,#+12
          CFI FunCall malloc
        BL       malloc
        MOVS     R4,R0
//    6 
//    7 	if (me != NULL) {
        CMP      R4,#+0
        BEQ.N    ??IO2Sensor_Create_0
//    8 		IO2Sensor_Init(me);
        MOVS     R0,R4
          CFI FunCall IO2Sensor_Init
        BL       IO2Sensor_Init
//    9 	}
//   10 
//   11 	return me;
??IO2Sensor_Create_0:
        MOVS     R0,R4
        POP      {R4,PC}          ;; return
//   12 }
          CFI EndBlock cfiBlock0
//   13 

        SECTION `.text`:CODE:NOROOT(1)
          CFI Block cfiBlock1 Using cfiCommon0
          CFI Function IO2Sensor_Destroy
        THUMB
//   14 void IO2Sensor_Destroy(IO2Sensor* const me) {
IO2Sensor_Destroy:
        PUSH     {R4,LR}
          CFI R14 Frame(CFA, -4)
          CFI R4 Frame(CFA, -8)
          CFI CFA R13+8
        MOVS     R4,R0
//   15 	if (me != NULL) {
        CMP      R4,#+0
        BEQ.N    ??IO2Sensor_Destroy_0
//   16 		free(me);
        MOVS     R0,R4
          CFI FunCall free
        BL       free
//   17 		//IO2Sensor_Cleanup(me);
//   18 	}
//   19 }
??IO2Sensor_Destroy_0:
        POP      {R4,PC}          ;; return
          CFI EndBlock cfiBlock1
//   20 

        SECTION `.text`:CODE:NOROOT(1)
          CFI Block cfiBlock2 Using cfiCommon0
          CFI Function IO2Sensor_Init
          CFI NoCalls
        THUMB
//   21 void IO2Sensor_Init(IO2Sensor* const me) {
//   22 	me->gimmeO2Conc = IO2Sensor_gimmeO2Conc;
IO2Sensor_Init:
        ADR.W    R1,IO2Sensor_gimmeO2Conc
        STR      R1,[R0, #+4]
//   23 	me->gimmeO2Flow = IO2Sensor_gimmeO2Flow;
        ADR.W    R1,IO2Sensor_gimmeO2Flow
        STR      R1,[R0, #+8]
//   24 }
        BX       LR               ;; return
          CFI EndBlock cfiBlock2
//   25 

        SECTION `.text`:CODE:NOROOT(2)
          CFI Block cfiBlock3 Using cfiCommon0
          CFI Function IO2Sensor_gimmeO2Conc
        THUMB
//   26 int IO2Sensor_gimmeO2Conc(IO2Sensor* const me) {
IO2Sensor_gimmeO2Conc:
        PUSH     {R4,LR}
          CFI R14 Frame(CFA, -4)
          CFI R4 Frame(CFA, -8)
          CFI CFA R13+8
        MOVS     R4,R0
//   27 	return me->gimmeO2Conc(me);
        MOVS     R0,R4
        LDR      R1,[R4, #+4]
          CFI FunCall
        BLX      R1
        POP      {R4,PC}          ;; return
//   28 }
          CFI EndBlock cfiBlock3
//   29 

        SECTION `.text`:CODE:NOROOT(2)
          CFI Block cfiBlock4 Using cfiCommon0
          CFI Function IO2Sensor_gimmeO2Flow
        THUMB
//   30 int IO2Sensor_gimmeO2Flow(IO2Sensor* const me) {
IO2Sensor_gimmeO2Flow:
        PUSH     {R4,LR}
          CFI R14 Frame(CFA, -4)
          CFI R4 Frame(CFA, -8)
          CFI CFA R13+8
        MOVS     R4,R0
//   31 	return me->gimmeO2Flow(me);
        MOVS     R0,R4
        LDR      R1,[R4, #+8]
          CFI FunCall
        BLX      R1
        POP      {R4,PC}          ;; return
//   32 }
          CFI EndBlock cfiBlock4

        SECTION `.iar_vfe_header`:DATA:NOALLOC:NOROOT(2)
        SECTION_TYPE SHT_PROGBITS, 0
        DATA
        DC32 0

        END
// 
// 78 bytes in section .text
// 
// 78 bytes of CODE memory
//
//Errors: none
//Warnings: none
