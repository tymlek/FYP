...
int main (int argc, char** argv)
{
    ...
    const int readingFrequency(argc >= 2 ? atoi(argv[1]) : 10);  // in seconds
    const int numOfReadings(argc >= 3 ? atoi(argv[2]) : 5);
    const double refVoltage(argc >= 4 ? atof(argv[3]) : 3.29);   // in Volts
